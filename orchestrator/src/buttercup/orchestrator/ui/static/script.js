...
function switchTab(tabName) {
    ...
    } else if (tabName === 'static-analysis') {
	    loadStaticAnalysis();
    }
}
...
function updatePageTitle() {
    const baseTitle = 'Talonera - Firmware Security Analysis';
...
function displayStaticClusters() {
    ...
    if (!clusters || clusters.length === 0) {
        container.innerHTML = '<div class="empty-state"><p>No static analysis results found</p></div>';
        return;
    }
    ...
    clusters.forEach(cluster => {
        const severityIcon = cluster.severity === 'error' ? '🔴' : cluster.severity === 'warning' ? '⚠️' : 'ℹ️';
        const fileName = cluster.file_paths[0].split('/').pop();

        html += `
            <div class="cluster-card ${cluster.severity}">
                <div class="cluster-header">
                ...
                <div class="cluster-body">
                    ...
                    <div class="cluster-details" id="static-cluster-${cluster.cluster_id}" style="display: none;">
                        ...
                        <ul>
                            ${cluster.file_paths.map(path => `<li><code>${escapeHtml(path)}</code></li>`).join('')}
                        </ul>

                        <h4>🔍 Individual Findings (${cluster.findings.length})</h4>
                        <table class="findings-table">
                            <thead>
                                <tr><th>Tool</th><th>File</th><th>Line</th><th>Message</th></tr>
                            </thead>
                            <tbody>
        ...
        html += `
            <div class="file-group">
                ...
                <div id="file-${fileName}" class="file-findings">
                    <table class="findings-table">
                        <thead>
                            <tr><th>Severity</th><th>Line</th><th>Tool</th><th>Rule</th><th>Message</th></tr>
                        </thead>
                        <tbody>
                            ${fileFindings.map(f => {
                                ...
                                return `
                                    <tr class="${f.severity}">
                                        <td>${icon} <span class="severity-badge ${f.severity}">${f.severity}</span></td>
